...
class PayslipController extends Controller
{
    ...
    function payslip_download(Request $request)
    {
        ...
        $dompdf = new Dompdf();
        $html_content = view('manager.payslip.invoice_with_assessment', ['invoice_id' => $request->invoice_id, 'user_id' => $request->user_id])->render();
    ...
    function payslip_send_to_email(Request $request)
    {
        ...
        $dompdf = new Dompdf();
        $html_content = view('manager.payslip.invoice', ['invoice_id' => $request->invoice_id, 'user_id' => $request->user_id])->render();
        ...
        try {
            if($attachment){
                Mail::send('manager.payslip.invoice_with_assessment', $page_data, function ($message) use ($payslip, $user, $payslip_file_url, $file_extension, $fileData) {
                ...
                Mail::send('manager.payslip.invoice_with_assessment', $page_data, function ($message) use ($payslip, $user) {
                    $message->to($user->email, $user->name)
                        ->subject('Monthly Salary of ' . date('1 M - t M, Y', strtotime($payslip->month_of_salary)) . ' - Creativeitem');
                });
            }
