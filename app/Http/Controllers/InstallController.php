...
class InstallController extends Controller
{
    ...
    public function step3(Request $request) {
        ...
        if ($data) {
            ...
            if ($db_connection == 'success') {
                $this->configure_database($hostname, $username, $password, $dbname);
    ...
    public function configure_database($hostname, $username, $password, $dbname) {
        // write database.php
        $data_db = file_get_contents(base_path('config/database.php'));
        $data_db = str_replace('db_name',    $dbname,    $data_db);
    ...
    public function finalizingSetup(Request $request) {
        ...
        if ($data) {
            ...
            $admin_data['present_address']      = $data['admin_address'];
    ...
    public function configure_routes() {
        $data_routes = file_get_contents(base_path('routes/install.php'));
        $data_routes = str_replace("Route::get('/'",    "Route::get('/install_ended'",    $data_routes);
        file_put_contents(base_path('routes/install.php'), $data_routes);
    }
}
