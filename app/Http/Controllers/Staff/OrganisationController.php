...
class OrganisationController extends Controller
{
    ...
    function index(){
        ...
        $all_users_structure = DB::select("WITH RECURSIVE ReportingStructure AS (
          ...
          SELECT id, manager as parent, case when photo is null then CONCAT('<img src=/hr/public/uploads/user-image/placeholder/placeholder.png height=42px></br> ', name) else CONCAT('<img src=/hr/public/uploads/user-image/', photo,' height=42px></br> ', name) end as name 
          FROM ReportingStructure");
        
        $page_data['all_users_structure'] = (array)$all_users_structure;

        $page_data['single_structure'] = DB::select("WITH RECURSIVE EmployeeStructure AS ( SELECT id, name, manager, photo FROM users WHERE id = ". auth()->user()->id ." UNION ALL SELECT u.id, u.name, u.manager, u.photo FROM users u JOIN EmployeeStructure es ON u.id = es.manager ) SELECT id, case when photo is null then CONCAT('<img src=/hr/public/uploads/user-image/placeholder/placeholder.png height=42px></br> ', name) else CONCAT('<img src=/hr/public/uploads/user-image/', photo,' height=42px></br> ', name) end as name ,case when manager is NULL THEN 0 ELSE manager END as parent FROM EmployeeStructure");
