<li id="task-list{{ $task->id }}" class="@if ($task->is_completed == 1) completed @endif">
    <div class="">
        <input onchange="actionTo('{{ route('admin.task.completion', ['id' => $task->id]) }}')" class="task-checkbox" type="checkbox"
    ...
    <div class="archive-section">
        @if($task->status == 'running')
            <button onclick="actionTo('{{ route('admin.task.status', ['id' => $task->id]) }}')" type="button" class="btn p-0"
