@extends('index')
@push('title', get_phrase('Employee Of The Quarter'))
@push('meta')
@endpush
@push('css')
@endpush
...
<div class="mainSection-title">
    <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
        <div class="d-flex flex-column">
            <h4>{{ get_phrase('Employee Of The Quarter') }}</h4>
            <ul class="d-flex align-items-center eBreadcrumb-2">
                <li><a href="#">{{ get_phrase('Dashboard') }}</a></li>
                <li><a href="#">{{ get_phrase('Employee Of The Quarter') }}</a></li>
            </ul>
        </div>
        <div class="export-btn-area d-flex ">
            <a href="#" class="export_btn" onclick="showRightModal('{{ route('right_modal', ['view_path' => 'admin.awards.eotq.add']) }}', '{{ __('Add Employee Of The Quarter') }}')">
                <i class="bi bi-plus me-2"></i>
                <span class="d-none d-sm-inline-block">{{ get_phrase('Add Employee Of The Quarter') }}</span>
            </a>
...
<div class="row">
    <div class="col-md-12">
        <div class="eSection-wrap">
            <p class="column-title mb-2">{{get_phrase('Employee Of The Quarter')}}</p>
                ...
                <table class="table eTable">
                    <thead>
                        <tr>
                            ...
                            <th class="">{{ get_phrase('Quarter') }}</th>
                            <th class="">{{ get_phrase('Year') }}</th>
                            <th class="">{{ get_phrase('Visibility') }}</th>
                            <th class="">{{ get_phrase('Actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($eotqs as $key => $eotq)
                        <tr>
                            <td>
                                {{ ++$key }}
                            </td>
                            ...
                            <td>
                                {{ date("F", mktime(0, 0, 0, $eotq->month, 10)); }}
                            </td>
                            <td>
                                {{ $eotq->year }}
                            </td>
                            <td>
                                {{ $eotq->visibility ? 'Visible' : 'Hidden' }}
                            </td>
                            <td>
                                <a href="#" onclick="showRightModal('{{ route('right_modal', ['view_path' => 'admin.awards.eotq.edit', 'eotq_id' => $eotq->id]) }}', '{{ __('Edit Employee Of The Quarter') }}')" class="btn btn p-0 px-1" title="{{ get_phrase('Edit') }}" data-bs-toggle="tooltip">
                                ...
                                <a href="#" onclick="confirmModal('{{ route('admin.eotq.delete', $eotq->id) }}')" class="btn btn p-0 px-1" title="{{ get_phrase('Delete') }}" data-bs-toggle="tooltip">
