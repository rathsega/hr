    ...
    <div class="mainSection-title">
        <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
            ...
            @php
                ...
                $sick_leaves = App\Models\Leave_application::where('user_id', auth()->user()->id)->where('leave_type','sick_leave')->whereNotIn('leave_applications.status', ['hr_rejected', 'manager_rejected','cancelled'])->get();
                $casual_leaves = App\Models\Leave_application::where('user_id', auth()->user()->id)->where('leave_type','casual_leave')->whereNotIn('leave_applications.status', ['hr_rejected', 'manager_rejected','cancelled'])->get();
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td class="text-center position-relative w-80px">
                                                                        ...
                                                                        <div class="contant-overlay">
                                                                            ...
                                                                            @endif
                                                                            @if($leave_report->user_id == auth()->user()->id && $leave_report->status == 'pending')
                                                                            ...
                                                                            @endif
                                                                            @if($leave_report->user_id == auth()->user()->id && in_array($leave_report->status, array('manager_approved', 'hr_approved')))
                                                                                <div class="contant-overlay">
                                                                                    <a href="#" onclick="confirmModal('{{ route('admin.leave.report.cancel', ['id'=>$leave_report->id, 'leave_type'=>$leave_report->leave_type, 'from_date'=>$leave_report->from_date, 'to_date'=>$leave_report->to_date]) }}')" type="button" class="btn px-1 py-0" title="{{ get_phrase('Cancel') }}"
                                                                                        data-bs-toggle="tooltip">
            ...
            <div class="eSection-wrap">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <form action="{{ route('admin.leave.report.store') }}" method="post" enctype="multipart/form-data">
                            @Csrf
