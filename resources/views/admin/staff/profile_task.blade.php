...
<ul class="nav nav-tabs eNav-Tabs-custom p-0" id="myTab" role="tablist">
    <li class="nav-item" role="presentation">
        <button class="nav-link @if ($task_type == 'running') active @endif" onclick="redirectTo('{{ route('admin.staff.profile',['tab' => 'task', 'user_id' => $user->id, 'task_type' => 'running']) }}')">
            {{get_phrase('Running Tasks')}}
            <span></span>
        </button>
    </li>
    <li class="nav-item" role="presentation">
        <button class="nav-link @if ($task_type == 'archive') active @endif" onclick="redirectTo('{{ route('admin.staff.profile',['tab' => 'task', 'user_id' => $user->id, 'task_type' => 'archive']) }}')">
