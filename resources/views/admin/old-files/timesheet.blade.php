...
@php
	...
	$timestamp_of_my_first_entry = DB::table('timesheets')->where('user_id', $user_id)->orderBy('id', 'asc')->value('from_date');
...
<div class="row">
	<div class="col-md-7">
		<div class="eSection-wrap">
			<div class="row">
				...
				<div class="col-md-6">
					<form action="{{route('admin.timesheet')}}" method="get">
						<!-- Month chooser -->
						<div class="fpb-7">
			...
			<div class="accordion custom-accordion" id="accordionOfMonth">
				...
				@for($d = date('t', $selected_timestamp_of_month); 1 <= $d; $d--)
					@php
						...
						$working_logs = DB::table('timesheets')->where('user_id', $user_id)->where('from_date', '>=', $from_date)->where('to_date', '<', $to_date)->orderBy('id', 'desc')->get();

						$attendance_details = DB::table('attendances')->where('user_id', $user_id)->where('checkin', '>=', $from_date)->where('checkin', '<=', $to_date)->first();
	...
	<div class="col-md-5">
		<div class="eSection-wrap">
			<div class="fpb-7">
				@php $my_task = DB::table('tasks')->where('user_id', $user_id)->first(); @endphp
		...
		<div class="eSection-wrap">
			<form action="{{route('admin.timesheet.add_working_log')}}" method="post">
				@Csrf
