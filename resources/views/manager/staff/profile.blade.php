        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
            <div class="d-flex flex-column">
                ...
                <ul class="d-flex align-items-center eBreadcrumb-2">
                    ...
                    <li><a href="{{route('manager.staffs')}}">{{ get_phrase('Employee') }}</a></li>
                    <li><a href="#">{{ get_phrase('Profile') }}</a></li>
                </ul>
            </div>
            <div class="export-btn-area d-flex ">
                <a href="{{route('manager.staffs')}}" class="export_btn ms-3">
                ...
                <div class="user-details-info mw-unset">
                    ...
                    <ul class="nav nav-tabs eNav-Tabs-custom" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['info', $user->id])}}');" class="nav-link @if($tab == 'info') active @endif" type="button">
                                {{ get_phrase('Basic Info') }}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['task', $user->id])}}');" class="nav-link  @if($tab == 'task') active @endif" type="button">
                                {{get_phrase('Task')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['attendance', $user->id])}}');" class="nav-link @if($tab == 'attendance') active @endif" type="button">
                                {{get_phrase('Attendance')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['timesheet', $user->id])}}');" class="nav-link @if($tab == 'timesheet') active @endif" type="button">
                                {{get_phrase('Timesheet')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['payslip', $user->id])}}');" class="nav-link @if($tab == 'payslip') active @endif" type="button">
                                {{get_phrase('Payslip')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['assessment', $user->id])}}');" class="nav-link @if($tab == 'assessment') active @endif" type="button">
                                {{get_phrase('Assessment')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['performance', $user->id])}}');" class="nav-link @if($tab == 'performance') active @endif" type="button">
                                {{get_phrase('Performance')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['leave', $user->id])}}');" class="nav-link @if($tab == 'leave') active @endif" type="button">
                                {{get_phrase('Leave')}}
                                <span></span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button onclick="redirectTo('{{route('manager.staff.profile', ['inventory', $user->id])}}');" class="nav-link @if($tab == 'inventory') active @endif" type="button">
                    ...
                    <div class="tab-content eNav-Tabs-content" id="myTabContent">
                        <div class="tab-pane fade show active">
                            <div class="eForm-layouts">
                                @include('manager.staff.profile_'.$tab)
                            </div>
                        </div>
                    </div>
                </div>
