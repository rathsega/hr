...
<ul class="nav nav-tabs eNav-Tabs-custom p-0" id="myTab" role="tablist">
    <li class="nav-item" role="presentation">
        <button class="nav-link @if ($task_type == 'running') active @endif" onclick="redirectTo('{{ route('manager.staff.profile',['tab' => 'task', 'user_id' => $user->id, 'task_type' => 'running']) }}')">
            {{get_phrase('Running Tasks')}}
            <span></span>
        </button>
    </li>
    <li class="nav-item" role="presentation">
        <button class="nav-link @if ($task_type == 'archive') active @endif" onclick="redirectTo('{{ route('manager.staff.profile',['tab' => 'task', 'user_id' => $user->id, 'task_type' => 'archive']) }}')">
...
<div class="tab-content eNav-Tabs-content profile-tab-content p-0" id="myTabContent">
    ...
    <ul class="mb-2">
        <li class="mx-2">
            <form action="{{ route('manager.task.store', $user->id) }}" method="post" class="ajaxForm resetable w-100">
                @Csrf
