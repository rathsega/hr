    ...
    <div class="row">
        <div class="col-md-8">
            <div class="eSection-wrap">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <form action="{{ route('manager.payslip') }}" method="get" id="filterForm">
                            <div class="row mb-4">
                                <div class="col-md-6">
                                ...
                                <tbody>
                                    @foreach ($users as $user)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if ($payslip->count() == 0)
                                                    <a href="{{ route('manager.payslip', ['user_id' => $user->id, 'year' => $selected_year, 'month' => $selected_month]) }}"
                                                ...
                                                @else
                                                    <a href="{{ route('manager.payslip', ['id' => $payslip->value('id')]) }}" class="btn btn p-0 px-1"
                                                        title="{{ get_phrase('Edit Invoice') }}" data-bs-toggle="tooltip">
                                                        <i class="fi-rr-blog-pencil"></i>
                                                    </a>

                                                    <a href="#"
                                                        onclick="confirmModal('{{ route('manager.payslip.send', ['invoice_id' => $payslip->value('id'), 'user_id' => $user->id]) }}')"
                                                    ...
                                                    <a href="#" onclick="confirmModal('{{ route('manager.payslip.delete', ['id' => $payslip->value('id')]) }}')"
        ...
        <div class="col-md-4">
            @if (isset($_GET['user_id']) && $_GET['user_id'] > 0)
                @include('manager.payslip.add')
            @endif
            @if (isset($_GET['id']) && $_GET['id'] > 0)
                @include('manager.payslip.edit')
            @endif
        </div>
    </div>
@endsection
