<h4 class="column-title">{{ get_phrase('Give attendance') }}</h4>
    <form action="{{ route('manager.attendance.store') }}" method="post" class="current-location-form">
        @Csrf
        <div class="row">
            <div class="col-md-12">
            ...
            <div class="col-md-6">
                <div class="fpb-7">
                    ...
                    <input type="datetime-local" name="time" value="{{ date('Y-m-d H:i') }}" class="form-control eForm-control date-time-picker" max="<?php echo date('Y-m-d\TH:i:s'); ?>"
                        id="eInputDateTime" />
                </div>
            </div>
            <div class="col-md-12">
