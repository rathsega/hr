<form action="{{ route('manager.my.profile.update', $user->id) }}" method="post" enctype="multipart/form-data">
    @Csrf
        ...
        <div class="col-md-6">
            <div class="fpb-7">
                ...
                <select name="role" class="form-select eForm-select select2" required>
                    <option value="manager" @if ($user->role == 'manager') selected @endif>{{ get_phrase('Manager') }}</option>
                </select>
            </div>
        </div>
        <div class="col-md-6">
