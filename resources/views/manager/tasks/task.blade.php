<li class="position-relative mx-2 @if ($task->is_completed == 1) completed @endif" id="task-list{{ $task->id }}">
    ...
    <div class="task-checkbox-content">
        <input onchange="actionTo('{{ route('manager.task.completion', ['id' => $task->id]) }}')" class="task-checkbox" type="checkbox" data-bs-toggle="tooltip"
            title="{{ get_phrase('Mark As Completed') }}" @if ($task->is_completed == 1) checked @endif>
    </div>
    <div class="task-area-content w-100">
        <form action="{{ route('manager.task.update', $task->id) }}" method="post" class="ajaxForm task-from">
            @Csrf
    ...
    <div class="archive-section">
        ...
        @if ($task->status == 'running')
            <button onclick="actionTo('{{ route('manager.task.status', ['id' => $task->id]) }}')" type="button" class="btn p-0" title="{{ get_phrase('Mark as Archive') }}"
                data-bs-toggle="tooltip">
                <i class="fi-rr-archive"></i>
            </button>
        @else
            <button onclick="actionTo('{{ route('manager.task.status', ['id' => $task->id]) }}')" type="button" class="btn px-1 py-0" title="{{ get_phrase('Restore') }}"
                data-bs-toggle="tooltip">
        ...
        <button class="btn px-1 py-0" onclick="actionTo('{{ route('manager.task.delete', ['id' => $task->id]) }}')" title="{{ get_phrase('Delete') }}" data-bs-toggle="tooltip">
