                                ...
                                ->where('from_date', '<=', strtotime(date('Y-m-t 23:59:59', $timestamp))) ->orderBy('id', 'desc');
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center position-relative w-80px">
                                            ...
                                            @elseif($leave_report->status == 'cancelled')
                                            <span class="badge bg-success">{{get_phrase('Cancelled')}}</span>
                                            @endif
                                            ...
                                            @endif
                                            @if(in_array($leave_report->status, array('manager_approved', 'hr_approved')))
                                                <div class="contant-overlay">
                                                    <a href="#" onclick="confirmModal('{{ route('staff.leave.report.cancel', ['id'=>$leave_report->id, 'leave_type'=>$leave_report->leave_type, 'from_date'=>$leave_report->from_date, 'to_date'=>$leave_report->to_date]) }}')" type="button" class="btn px-1 py-0" title="{{ get_phrase('Cancel') }}"
