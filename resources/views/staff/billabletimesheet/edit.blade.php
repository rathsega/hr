@php
    $timesheet = App\Models\Billable_timesheets::where('id', $id)->first();
@endphp
<h4 class="column-title">{{ get_phrase('Update Timesheets') }}</h4>
<form action="{{ route('staff.billabletimesheet.update', [$timesheet->id, $timesheet->timesheet]) }}" method="post"  enctype="multipart/form-data" id="timesheetForm" class="current-location-form">
    @Csrf
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="fpb-7">
                ...
                <input type="date" value="{{ date('Y-m-d', strtotime($timesheet->from_date)) }}" max="<?php echo date('Y-m-d'); ?>" name="from_date" class="form-control eForm-control" id="from_date" />
            </div>
        </div>
        <div class="col-md-12">
            <div class="fpb-7">
                <label for="date" class="eForm-label">{{get_phrase('To Date')}}</label>
                <input type="date" value="{{ date('Y-m-d', strtotime($timesheet->to_date)) }}" max="<?php echo date('Y-m-d'); ?>" name="to_date" class="form-control eForm-control" id="to_date" />
            </div>
        </div>
        <div class="col-md-12">
            <div class="fpb-7">
                <label for="leave_dates" class="eForm-label">{{get_phrase('Leave Dates')}}</label>
                <textarea type="leave_dates" name="leave_dates" class="form-control eForm-control" id="leave_dates" required rows="6" maxlength="150">{{$timesheet->leave_dates}}</textarea>
        ...
        <div class="col-md-10">
            <div class="fpb-7">
                ...
                @if($timesheet->timesheet)
                                Download : <a href="./../public/uploads/billabletimesheets/{{$timesheet->timesheet }}" download class="btn btn p-0 px-1" title="{{ get_phrase('Download') }}" data-bs-toggle="tooltip">
