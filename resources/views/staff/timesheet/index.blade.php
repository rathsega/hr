                        ...
                        <div class="accordion custom-accordion" id="accordionExample">
                            ...
                            @for ($day = $total_days_of_this_month; $day >= 1;  $day--)
                                @php
                                    ...
                                    $timesheet_staffs = App\Models\Timesheet::join('users', 'users.id', '=', 'timesheets.user_id')->where('users.id', auth()->user()->id)->where('timesheets.from_date', '>=', $start_timestamp)->where('timesheets.to_date', '<=', $end_timestamp)->select('timesheets.user_id', DB::raw('count(*) as user_work_list'))->groupBy('timesheets.user_id')->orderBy('users.sort');
