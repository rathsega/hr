...
@php
	$timestamp_of_my_first_attendance = DB::table('attendances')->where('user_id', auth()->user()->id)->orderBy('id', 'asc')->value('checkin');
...
<div class="row">
	<div class="col-md-7">
		<div class="eSection-wrap">
			<div class="row">
				...
				<div class="col-md-6">
					<form action="{{route('staff.attendance')}}" method="get">
						<label class="eForm-label">Selected Month</label>
						<select onchange="$(this).parent().submit();" name="date" class="form-select eForm-select eChoice-multiple-without-remove">
		...
		<div class="eSection-wrap">
			...
			<div class="table-responsive">
				<table class="table eTable">
					...
					<tbody>
						@php
							$att_reports = DB::table('attendances')->where('user_id', auth()->user()->id)->where('checkin', '>=', $start_timestamp)->where('checkin', '<=', $end_timestamp)->orderBy('id', 'desc')->get();
						@endphp
	...
	<div class="col-md-5">
				...
				<table class="table eTable">
					...
					<tbody>
						<tr>
							...
							<td>
								{{DB::table('attendances')->where('user_id', auth()->user()->id)->where('late_entry', 1)->where('checkin', '>=', $start_timestamp)->where('checkin', '<=', $end_timestamp)->get()->count()}}
							</td>
							<td>
								{{DB::table('attendances')->where('user_id', auth()->user()->id)->where('late_entry', 1)->where('checkin', '>=', $start_timestamp_of_year)->where('checkin', '<=', $end_timestamp_of_year)->get()->count()}}
							</td>
						</tr>
						<tr>
							<td>
							...
							<td>
								{{DB::table('attendances')->where('user_id', auth()->user()->id)->where('early_leave', 1)->where('checkin', '>=', $start_timestamp)->where('checkin', '<=', $end_timestamp)->get()->count()}}
							</td>
							<td>
								{{DB::table('attendances')->where('user_id', auth()->user()->id)->where('early_leave', 1)->where('checkin', '>=', $start_timestamp_of_year)->where('checkin', '<=', $end_timestamp_of_year)->get()->count()}}
							</td>
						</tr>
						<tr>
							<td>
							...
							<td>
								@php
									$absent_monthly = DB::table('leave_applications')->where('user_id', auth()->user()->id)->where('from_date', '>=', $start_timestamp)->where('from_date', '<=', $end_timestamp)->where('status', 'approved');
								@endphp
							...
							<td>
								@php
									$absent_yearly = DB::table('leave_applications')->where('user_id', auth()->user()->id)->where('from_date', '>=', $start_timestamp_of_year)->where('from_date', '<=', $end_timestamp_of_year)->where('status', 'approved');
								@endphp
						...
						<tr>
							...
							<td>
								{{DB::table('leave_applications')->where('user_id', auth()->user()->id)->where('from_date', '>=', $start_timestamp)->where('from_date', '<=', $end_timestamp)->get()->count()}}
							</td>
							<td>
								{{DB::table('leave_applications')->where('user_id', auth()->user()->id)->where('from_date', '>=', $start_timestamp_of_year)->where('from_date', '<=', $end_timestamp_of_year)->get()->count()}}
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div class="eSection-wrap">
			...
			<form action="{{route('staff.attendance.add')}}" method="post">
				@Csrf
		...
		<div class="eSection-wrap">
			...
			<div class="tab-content eNav-Tabs-content" id="myTabContent" >
				<div class="tab-pane fade show active" id="cHome" role="tabpanel" aria-labelledby="cHome-tab">
					<p>
						<table class="table eTable">
							...
							<tbody>
								@foreach(DB::table('leave_applications')->where('user_id', auth()->user()->id)->take(20)->orderBy('id', 'desc')->get() as $leave_requests)
									<tr>
										<td>
				...
				<div class="tab-pane dff fade" id="cProfile" role="tabpanel" aria-labelledby="cProfile-tab" >
					<form action="{{route('staff.leave_application.add')}}" method="post">
						@Csrf
